...
| 案例 | 布局 | 模式 | 结果 | 路径 | 总次数 |
| --- | --- | --- | --- | --- | --- |
| A | transpose | raw | 预期错读 | Raw Fast | 10 |
| A2 | padded | dispatch | 正确 | Fast | 10 |
| B | contiguous | dispatch | 正确 | Fast | 10 |

## 提取出的访问与布局条件

| 案例 | 输入 | 分析状态 | 输入地址偏移 | 读取 mask | 候选条件 |
| --- | --- | --- | --- | --- | --- |
| A | X | Supported | `tl.program_id(0) * B + tl.arange(0, B)` | `idx < M * N` | `size(0) == 1 或 X.stride(0) == size(1)；size(1) == 1 或 X.stride(1) == 1` |
| A2 | X | Supported | `tl.program_id(0) * S0 + tl.arange(0, B)` | `col < N` | `X.stride(1) == 1` |
| B | X | Supported | `tl.program_id(0) * B + tl.arange(0, B)` | `idx < N` | `X.stride(0) == 1` |
| D | X | Supported | `tl.program_id(axis=0) * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)` | `offsets < n_elements` | `X.stride(0) == 1` |
| D | Y | Supported | `tl.program_id(axis=0) * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)` | `offsets < n_elements` | `Y.stride(0) == 1` |
...
| --- | --- | --- | --- | --- |
| A | size(0) == 1 或 X.stride(0) == size(1) | X 的 load：`scenarios/kernels.py:15` | x[row, col] | X + idx 与二维行优先逻辑索引对应 |
| A | size(1) == 1 或 X.stride(1) == 1 | X 的 load：`scenarios/kernels.py:15` | x[row, col] | X + idx 的相邻列地址差为 1 |
| A2 | X.stride(1) == 1 | X 的 load：`scenarios/kernels.py:25` | x[row, col] | X + row*S0 + col 的相邻列地址差为 1；S0 来自输入 stride(0) |
| B | X.stride(0) == 1 | X 的 load：`scenarios/kernels.py:34` | x[i] | X + idx 的相邻元素地址差为 1 |
| D | X.stride(0) == 1 | x_ptr 的 load：`scenarios/external_add.py:19` | x[i] | x_ptr + offsets 对应 x 的逐元素索引 |
| D | Y.stride(0) == 1 | y_ptr 的 load：`scenarios/external_add.py:20` | y[i] | y_ptr + offsets 对应 y 的逐元素索引 |
