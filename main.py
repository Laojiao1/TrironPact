...
def main() -> int:
    ...
    parser.add_argument("command", choices=("ir", "access-ir", "stage-ir", "stage-candidates", "stage-boundary", "demo"))
    ...
    parser.add_argument("--quick", action="store_true", help="第四阶段仅运行每例一个代表性边界")
    ...
    elif args.command == "stage-boundary":
        from pact.boundary_report import build_boundary_report, render_boundary_report

        payload = build_boundary_report(quick=args.quick)
    else:
        payload = demo(args.repeats)
    rendered_json = json.dumps(payload, ensure_ascii=False, indent=2) + "\n"
    if args.output:
        output = Path(args.output)
        if output.suffix == ".md" and args.command in ("demo", "stage-ir", "stage-candidates", "stage-boundary"):
            renderer = {"demo": render_markdown, "stage-ir": render_stage_report, "stage-candidates": render_candidate_report, "stage-boundary": render_boundary_report}[args.command]
        ...
        print(render_markdown(payload) if args.command == "demo" else (render_stage_report(payload) if args.command == "stage-ir" else (render_candidate_report(payload) if args.command == "stage-candidates" else (render_boundary_report(payload) if args.command == "stage-boundary" else rendered_json))))
    ...
    if args.command == "stage-boundary":
        return 0 if payload["go_boundary"] else 1
